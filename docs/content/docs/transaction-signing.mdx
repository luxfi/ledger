...
func signPChainTx(app *ledger.LedgerLux) error {
    ...
    addr, err := app.GetPubKey("m/44'/60'/0'/0/0", true, hrp, chainID)
    if err != nil {
        return err
    }
    ...
    // Sign transaction
    response, err := app.Sign("m/44'/60'/0'", []string{"0/0"}, stakingTx, nil)
    if err != nil {
        return err
    }
...
func signXChainTx(app *ledger.LedgerLux) error {
    ...
    response, err := app.Sign("m/44'/60'/0'", inputPaths, utxoTx, changePath)
    if err != nil {
        return err
    }
...
func blindSign(app *ledger.LedgerLux, txHash []byte) error {
    ...
    response, err := app.SignHash("m/44'/60'/0'", []string{"0/0"}, txHash)
    if err != nil {
...
func batchSign(app *ledger.LedgerLux, transactions [][]byte) ([]ledger.ResponseSign, error) {
    ...
    for i, tx := range transactions {
        ...
        response, err := app.Sign("m/44'/60'/0'", []string{"0/0"}, tx, nil)
        if err != nil {
...
func signMessage(app *ledger.LedgerLux, message string) error {
    ...
    // Sign the hash
    response, err := app.SignHash("m/44'/60'/0'", []string{"0/0"}, hash[:])
    if err != nil {
        return err
    }
...
func verifySignature(app *ledger.LedgerLux, response ledger.ResponseSign, txHash []byte) error {
    ...
    addr, err := app.GetPubKey("m/44'/60'/0'/0/0", false, "lux", "")
    if err != nil {
        return err
    }
    ...
    for path, sigBytes := range response.Signature {
        ...
        fmt.Printf("✓ Signature verified for path %s\n", path)
    }
...
func main() {
    ...
    addr, err := app.GetPubKey("m/44'/60'/0'/0/0", true, "P-lux", "")
    if err != nil {
        log.Fatal(err)
    }
