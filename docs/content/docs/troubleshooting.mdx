...
func signWithTimeout(app *ledger.LedgerLux, tx []byte, timeout time.Duration) error {
    ...
    go func() {
        resp, err := app.Sign("m/44'/60'/0'", []string{"0/0"}, tx, nil)
...
func validatePath(path string) error {
    ...
    for i, part := range parts {
        ...
        if i == 1 && num != 60 {
            fmt.Println("Warning: Non-standard coin type (expected 60 for Lux)")
        }
    }
...
var commonPaths = map[string]string{
    "P-Chain Primary":    "m/44'/60'/0'/0/0",
    "X-Chain Primary":    "m/44'/60'/0'/0/0",
    "C-Chain Primary":    "m/44'/60'/0'/0/0",
    "Change Address":     "m/44'/60'/0'/1/0",
