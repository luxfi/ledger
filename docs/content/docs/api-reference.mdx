...
**Example:**
```go
addr, err := app.GetAddress("m/44'/60'/0'/0/0", true)
if err != nil {
    log.Fatal(err)
}
fmt.Printf("Address: %s\n", addr.Address)
...
**Parameters:**
- `pathPrefix`: Common prefix for all paths (e.g., "m/44'/60'/0'")
...
**Parameters:**
- `path`: BIP44 path string (e.g., "m/44'/60'/0'/0/0")
